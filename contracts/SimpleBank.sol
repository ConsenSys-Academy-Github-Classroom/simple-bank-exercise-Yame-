...
contract SimpleBank {
    ...
    mapping (address => bool) private enrolledOnes;
    ...
    function enroll() public returns (bool){
      // 1. enroll of the sender of this transaction
        enrolledOnes[msg.sender] = true;
        emit LogEnrolled(msg.sender);
        return enrolledOnes[msg.sender];
    }

    function enrolled(address enrolAddress) view public returns (bool){
        return enrolledOnes[enrolAddress];
    }
    ...
    function deposit() public payable returns (uint) {
      ...
      // 2. Users should be enrolled before they can make deposits
        if( ! enrolledOnes[msg.sender] ){
            revert();
        }
        //require(!enrolled[msg.sender], "U need 2 be enrolled.");
